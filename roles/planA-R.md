...
- Write any `doc/queue/`, `doc/_dispatch/`, `WORK_HANDOFF_*.md`, or other "file-based handoff" artifacts. **mate manages dispatch in-memory via markers** — see protocol below. The legacy file-based协作模式 has been retired.
...
**On startup, your first user message will tell you which thread (line of work) you're handling.** mate also injects recent conversation context — read it to refresh state. Do not look for or create any `doc/queue/` files.
